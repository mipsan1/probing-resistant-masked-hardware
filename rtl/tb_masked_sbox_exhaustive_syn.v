// =====================================================================
// tb_masked_sbox_exhaustive_syn.v
// Exhaustive (256-input) testbench driven against the gate-level
// synthesized netlist (post-synthesis verification).
//
// Uses the same stimulus as tb_masked_sbox_exhaustive.v but
// instantiates the synthesized netlist (`masked_sbox_first_order`)
// from the Yosys output.
// =====================================================================

`ifndef TB_MASKED_SBOX_EXHAUSTIVE_SYN_V
`define TB_MASKED_SBOX_EXHAUSTIVE_SYN_V

`timescale 1ns/1ps

`include "/Users/ckim/Downloads/IEEE-Transactions-TIFS/rtl/masked_sbox_pkg.v"

module tb_masked_sbox_exhaustive_syn;
  ...
  initial begin
      ...
      forever begin
        ...
        if (valid_out_1 && !seen_valid_1) begin
          ...
          if ((y0_1 ^ y1_1) !== expected) begin
            $display("FAIL first-order (gate-level): input=0x%02h expected=0x%02h got=0x%02h (y0=0x%02h y1=0x%02h)",
      ...
      forever begin
        ...
        if (valid_out_2 && !seen_valid_2) begin
          ...
          if ((y0_2 ^ y1_2 ^ y2_2) !== expected) begin
            $display("FAIL second-order (gate-level): input=0x%02h expected=0x%02h got=0x%02h (y0=0x%02h y1=0x%02h y2=0x%02h)",
    ...
    $display("=================================================");
    $display("  Exhaustive test (256 inputs x 2 DUTs, gate-level):");
