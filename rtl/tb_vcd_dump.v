...
// tb_vcd_dump.v
// =====================================================================
// VCD-based power trace testbench for the full-chip masked AES round 1.
// Dumps y0_out, y1_out to VCD; post-processing computes HD-model
// power trace and TVLA / MI in Python.
//
// Usage:
//   iverilog -g2012 -I . -o /tmp/tb_vcd.vvp \
//     tb_vcd_dump.v masked_sbox_first_order.v masked_sbox_pkg.v \
//     masked_aes_round1.v
//   vvp /tmp/tb_vcd.vvp
//   python3 syn/vcd_to_hd.py sim/power.vcd sim/power_trace.txt
// =====================================================================

`ifndef TB_VCD_DUMP_V
`define TB_VCD_DUMP_V
...
module tb_vcd_dump;
    ...
    reg [7:0]   h_masks  [0:1119999];   // 10000 * 112
    ...
    // ----------------------------------------------------------------
    // VCD dump: only y0_out, y1_out, valid_out
    // ----------------------------------------------------------------
    initial begin
        $dumpfile("sim/power.vcd");
        $dumpvars(0, u_dut.y0_out);
        $dumpvars(0, u_dut.y1_out);
        $dumpvars(0, u_dut.valid_out);
    end
    ...
    initial begin
        ...
        for (n = 0; n < N_VECTORS; n = n + 1) begin : VEC_LOOP
            ...
            rk1_in = h_rk1[n];
            for (b = 0; b < 112; b = b + 1)
                r_in[b] = h_masks[n*112 + b];
            @(posedge clk);
            valid_in = 1'b1;
            @(posedge clk);
            valid_in = 1'b0;
            // Wait the 19-cycle pipeline latency (+1 cycle margin so the
            // output register settles before the next vector)
            @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk);
            @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk);
            @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk);
            @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk);
        end
        $display("VCD dumped %0d vectors", N_VECTORS);
        $finish;
    end
